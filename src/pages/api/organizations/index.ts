...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    if (body?.climate_project?.length > 0) {
      const create_climate_project = body.climate_project;
      body.climate_project = {
        create: create_climate_project,
      };
    } else {
      delete body.climate_project;
    }
    if (body?.environmental_impact?.length > 0) {
      const create_environmental_impact = body.environmental_impact;
      body.environmental_impact = {
        create: create_environmental_impact,
      };
    } else {
      delete body.environmental_impact;
    }
